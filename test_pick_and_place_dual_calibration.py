...
assembly_sequence = [[1, 1, [0, 0, 0.02], [0, -1, 0]],
                         ...
                         [1, 11, [0.02, 0, 0.02], [0, -1, 0]],
                         [1, 4, [0.00, 0, 0.02], [0, -1, 0]],
                         [0, 3, [-0.02, 0, 0.02], [0, -1, 0]],
                         [0, 5, [-0.02, 0, 0.02], [1, 0, 0]],
                         [1, 7, [0.02, 0, 0.02], [-1, 0, 0]],
                         [0, 8, [-0.02, 0, 0.02], [1, 0, 0]],
                         [1, 6, [0.00, 0, 0.02], [0, 0, -1]],
...
with open("Dual-robots/saved_plan_print_dual_0428_platform.json", "r") as f:
    data = json.load(f)
        ...
        if command["type"] == "pick_station":
            ...
            if data[i - 1]["type"] == "gripper" and data[i - 1]["activate"] == True:
                ...
                if data[i + 1]["type"] == "gripper" and data[i + 1]["activate"] == True:
                    ...
                    move_to_calibration(calibrated_poses[0], robotip, 0.05)
                    # info['type'] = 1
                    # send_capture_message(info)
                    ...
                    # while not_get_delta_pose:
                    #     time.sleep(0.1)
                    # move_to_calibration(calibrated_poses[0], robotip, 0.01)

                    # print("Press Enter to continue...")
                    # input()
                    # print("The program has resumed.")
                    move_to_calibration(calibrated_poses[1], robotip, 0.02)
                    # info['type'] = 2
                    # send_capture_message(info)
